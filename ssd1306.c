...
void ssd1306_draw_square(ssd1306_t *ssd, uint8_t x, uint8_t y) { // THALLES
...
void ssd1306_draw_thin_border(ssd1306_t *ssd) {
    ...
    // Desenha a borda de 1 pixel
    for (int x = 0; x < WIDTH; x++) {
        ssd1306_pixel(ssd, x, 0, true);               // Borda superior
        ssd1306_pixel(ssd, x, HEIGHT - 1, true);      // Borda inferior
    }
    for (int y = 0; y < HEIGHT; y++) {
        ssd1306_pixel(ssd, 0, y, true);              // Borda esquerda
        ssd1306_pixel(ssd, WIDTH - 1, y, true);      // Borda direita
    }
}
...
void ssd1306_draw_thick_border(ssd1306_t *ssd) {
    ...
    // Desenha a borda de 2 pixels
    for (int x = 0; x < WIDTH; x++) {
        ssd1306_pixel(ssd, x, 0, true);               // Borda superior (linha 1)
        ssd1306_pixel(ssd, x, 1, true);               // Borda superior (linha 2)
        ssd1306_pixel(ssd, x, HEIGHT - 2, true);      // Borda inferior (linha 1)
        ssd1306_pixel(ssd, x, HEIGHT - 1, true);      // Borda inferior (linha 2)
    }
    for (int y = 0; y < HEIGHT; y++) {
        ssd1306_pixel(ssd, 0, y, true);              // Borda esquerda (linha 1)
        ssd1306_pixel(ssd, 1, y, true);              // Borda esquerda (linha 2)
        ssd1306_pixel(ssd, WIDTH - 2, y, true);      // Borda direita (linha 1)
        ssd1306_pixel(ssd, WIDTH - 1, y, true);      // Borda direita (linha 2)
    }
}
...
void ssd1306_draw_thicker_border(ssd1306_t *ssd) {
    ...
    // Desenha a borda de 3 pixels
    for (int x = 0; x < WIDTH; x++) {
        ssd1306_pixel(ssd, x, 0, true);               // Borda superior (linha 1)
        ssd1306_pixel(ssd, x, 1, true);               // Borda superior (linha 2)
        ssd1306_pixel(ssd, x, 2, true);               // Borda superior (linha 3)
        ssd1306_pixel(ssd, x, HEIGHT - 3, true);      // Borda inferior (linha 1)
        ssd1306_pixel(ssd, x, HEIGHT - 2, true);      // Borda inferior (linha 2)
        ssd1306_pixel(ssd, x, HEIGHT - 1, true);      // Borda inferior (linha 3)
    }
    for (int y = 0; y < HEIGHT; y++) {
        ssd1306_pixel(ssd, 0, y, true);              // Borda esquerda (linha 1)
        ssd1306_pixel(ssd, 1, y, true);              // Borda esquerda (linha 2)
        ssd1306_pixel(ssd, 2, y, true);              // Borda esquerda (linha 3)
        ssd1306_pixel(ssd, WIDTH - 3, y, true);      // Borda direita (linha 1)
        ssd1306_pixel(ssd, WIDTH - 2, y, true);      // Borda direita (linha 2)
        ssd1306_pixel(ssd, WIDTH - 1, y, true);      // Borda direita (linha 3)
    }
}
